...
*Setup instructions for a Gitian build of Recrypt Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Recrypt
...
- [Building Recrypt Core](#building-recrypt-core)
...
Clone the git repositories for recrypt and Gitian.
...
```bash
sudo cp /home/debian/recrypt/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the recrypt repository under 'Fetch and create inputs' to install sources which require
...
To build Recrypt Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the recrypt repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/recrypt/.git/
...
For example:
```bash
URL=https://github.com/laanwj/recrypt.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit recrypt=${COMMIT} --url recrypt=${URL} ../recrypt/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit recrypt=${COMMIT} --url recrypt=${URL} ../recrypt/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit recrypt=${COMMIT} --url recrypt=${URL} ../recrypt/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-recrypt:
./bin/gbuild --commit recrypt=${COMMIT},cpp-eth-recrypt=${ETHCOMMIT} --url recrypt=${URL},cpp-eth-recrypt=${ETHURL} ../recrypt/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the recrypt git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../recrypt/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/recrypt
SIGPATH=/some/root/path/recrypt-detached-sigs

./bin/gbuild --url recrypt=${BTCPATH},signature=${SIGPATH} ../recrypt/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/recrypt-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/recrypt-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/recrypt-osx-build.assert
```
...
[recryptproject/gitian.sigs](https://github.com/recryptproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@recrypt.org) and he will commit them.
