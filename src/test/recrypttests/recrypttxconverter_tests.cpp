...
void checkResult(bool isCreation, std::vector<RecryptTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    RecryptTxConverter converter(transaction, NULL);
    ExtractRecryptTX recryptTx;
    BOOST_CHECK(converter.extractionRecryptTransactions(recryptTx));
    std::vector<RecryptTransaction> result = recryptTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(recrypttxconverter_tests, TestingSetup)
