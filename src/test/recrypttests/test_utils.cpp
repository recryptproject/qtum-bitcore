...
void initState(){
    ...
    const std::string dirRecrypt = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<RecryptState>(new RecryptState(dev::u256(0), RecryptState::openDB(dirRecrypt, hashDB, dev::WithExisting::Trust), dirRecrypt + "/recryptDB", dev::eth::BaseState::Empty));
...
RecryptTransaction createRecryptTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    RecryptTransaction txEth;
    if(recipient == dev::Address()){
        txEth = RecryptTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = RecryptTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<RecryptTransaction> txs){
