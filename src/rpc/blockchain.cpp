...
{
    if (pindexBestHeader->nHeight >= Params().GetConsensus().nLastPOWBlock)//tonirecrypt edit
        return 0;
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // recrypt
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // recrypt
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // recrypt
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // recrypt
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress recryptSenderAddress(request.params[2].get_str());
        if(recryptSenderAddress.IsValid()){
            CKeyID keyid;
            recryptSenderAddress.GetKeyID(keyid);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of recrypt addresses\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "callcontract",           &callcontract,           true,  {"address","data"} }, // recrypt
