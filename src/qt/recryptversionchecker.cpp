...
#define paternVersion "recrypt-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

RecryptVersionChecker::RecryptVersionChecker(QObject *parent) : QObject(parent)
{
...
bool RecryptVersionChecker::newVersionAvailable()
{
...
QList<Version> RecryptVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(RECRYPT_RELEASES)));
    QEventLoop event;
...
Version RecryptVersionChecker::getMaxReleaseVersion()
{
