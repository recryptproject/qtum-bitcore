...
bool ToHash160(const std::string& strRecryptAddress, std::string& strHash160)
{
    CBitcoinAddress recryptAddress(strRecryptAddress);
    if(recryptAddress.IsValid()){
        CKeyID keyid;
        recryptAddress.GetKeyID(keyid);
...
bool ToRecryptAddress(const std::string& strHash160, std::string& strRecryptAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress recryptAddress;
    recryptAddress.Set(keyid);
    if(recryptAddress.IsValid()){
        strRecryptAddress = recryptAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToRecryptAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToRecryptAddress(tokenEvent.receiver, tokenEvent.receiver);
