#include "recryptDGP.h"

void RecryptDGP::initDataEIP158(){
...
bool RecryptDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule RecryptDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t RecryptDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t RecryptDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t RecryptDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t RecryptDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool RecryptDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void RecryptDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void RecryptDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void RecryptDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void RecryptDGP::createParamsInstance(){
...
dev::Address RecryptDGP::getAddressForBlock(unsigned int blockHeight){
...
void RecryptDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void RecryptDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void RecryptDGP::parseStorageOneUint64(uint64_t& value){
...
void RecryptDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule RecryptDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
